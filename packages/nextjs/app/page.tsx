...
import { SUPPORTED_CHAINS, SUPPORTED_CHAINS_IDS, SUPPORTED_TOKENS } from "avail-nexus-sdk";
import type { SimulationResult, TransferParams } from "avail-nexus-sdk";
        ...
        <div className="absolute left-0 z-10 mt-2 w-[32rem] rounded-md shadow-lg bg-base-100 border p-4">
          ...
          <div className="mb-4 p-2 bg-primary/10 rounded">
            ...
            <div className="text-lg font-bold">
              ${totalFiat.toLocaleString(undefined, { maximumFractionDigits: 2 })}
            </div>
          </div>
              ...
              <div key={token.symbol} className="border rounded p-3">
                ...
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="text-sm text-gray-500">
                    Total: {Number(token.balance).toLocaleString(undefined, { maximumFractionDigits: 6 })} ($
                    {token.balanceInFiat.toLocaleString(undefined, { maximumFractionDigits: 2 })})
                  </div>
                </div>
                ...
                <div className="pl-8 space-y-2">
                  {token.breakdown.map((chainBalance, index) => {
                    ...
                    return (
                      <div
                        key={`${token.symbol}-${chainBalance.chain.id}-${index}`}
  ...
  return (
    <div className="mb-8 p-6 rounded-lg border bg-base-100">
      ...
      <div className="flex flex-col gap-4">
        ...
        {simulationError && <div className="p-2 bg-error/10 rounded text-error text-xs">Error: {simulationError}</div>}
        {simulationResult && (
        ...
        {transferError && <div className="p-2 bg-error/10 rounded text-error text-xs">Error: {transferError}</div>}
        {transferResult && (
          <div className="p-2 bg-success/10 rounded text-success text-xs whitespace-pre-wrap">
...
function Bridge() {
  ...
  const [txStatus, setTxStatus] = useState<string | null>(null);
  const [, setCurrentChainId] = useState<number | null>(null);
  const [allowanceState] = useState<any>(null);
  const [intentState] = useState<any>(null);
  ...
  return (
    <div className="mb-8 p-6 rounded-lg border bg-base-100">
      ...
      <div className="flex flex-col gap-4">
        <input className="input input-bordered" value={connectedAddress} disabled placeholder="Recipient address" />
        <EtherInput value={amount} onChange={setAmount} placeholder="Amount" />
        <select className="input input-bordered" value={token} onChange={e => setToken(e.target.value)}>
          {TOKEN_OPTIONS.map(t => (
            <option key={t.value} value={t.value}>
              {t.label}
            </option>
        ...
        {intentState && (
          <div className="p-2 bg-success/10 rounded text-success text-xs">Intent: {JSON.stringify(intentState)}</div>
        )}
        {bridgeError && <div className="p-2 bg-error/10 rounded text-error text-xs">Error: {bridgeError}</div>}
